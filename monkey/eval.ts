...
function evalStatements(stmts: ast.Statement[], env: object.Environment): object.MObject {
    let res = null
...
function evalBlockStatement(block: ast.BlockStatement, env: object.Environment): object.MObject {
    let res: object.MObject
...
function evalIfExpression(
    ...
    const condition = Eval(ie.condition, env) // -> Bool object
...
function evalWhileExpression(
    ...
    let ret = NULL
    while (Eval(we.condition, env).primitive) { // -> Bool object
        ret = Eval(we.body, env)
    }
    return ret
}
...
testEval('let a = 5; let b = 2 * a; while(a < b) {a = a + 1;}')